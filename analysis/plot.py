...
def main() -> None:
    ...
    # offsets_used.csv (written by fit_hybrid2.py) wins: it is what the mapping
    # itself was built with.
    offs = {}
    rep_path = OUT / "offsets_used.csv"
    ...
    for ep in eps:
        ...
        flags = []
        if ep in per_sync.index and per_sync.loc[ep, "r"] < 0.5:
            flags.append("low sync r")
    ...
    # Contact sheets: 6 columns, as many rows as the session needs (7 for 42).
    for kind in ("y", "x"):
        ncol = 6
        nrow = max(1, -(-len(panels[kind]) // ncol))
        fig, axes = plt.subplots(nrow, ncol, figsize=(24, 18 * nrow / 7), squeeze=False)
        ...
        fig.tight_layout(rect=[0, 0, 1, 1 - 0.025 * 7 / nrow])
