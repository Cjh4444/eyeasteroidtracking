function Lunar_Blast_v4()
%% ------------------------------------------------------------------------
%  Serial port settings
%  Update COM_PORT before running pilot testing.
%  macOS example: '/dev/tty.usbmodem114301'
...
%% ------------------------------------------------------------------------
%  LUMO fNIRS synchronisation settings
%  A second serial port is opened automatically (the last free port found by
%  serialportlist). An ASCII trigger code is written as uint16 immediately
%  before every stimulus event so LUMO data can be time-locked offline.
%
%  Trigger code table
%  ------------------
%  1  – Task start      (first epoch about to begin)
%  2  – Epoch onset     (asteroid begins moving, OBSERVATION trial)
%  3  – Epoch onset     (asteroid begins moving, TRACKING trial)
%  4  – Rest onset      (fixation cross appears)
%  5  – Deflected       (tracking threshold met, deflection animation)
%  6  – Planet impact   (tracking threshold NOT met)
%  7  – Observation end (observation epoch finished)
%  8  – Task complete   (all epochs done)
% -------------------------------------------------------------------------
LUMO_BAUD_RATE = 9600;
...
% -------------------------------------------------------------------------
DEFAULT_CONFIG_FILE = 'lunar_config.csv';
...
%% ------------------------------------------------------------------------
%  Subject ID — collected via a small standalone popup before anything else
% -------------------------------------------------------------------------
subjectID       = '';
subjectID_clean = '';
selectedPlanet  = [];
...
% ---- Show Subject ID dialog FIRST, before the main figure is created ----
...
%% ------------------------------------------------------------------------
%  Single persistent figure — welcome screen and game share this window
% -------------------------------------------------------------------------
fig = figure( ...
    'Name', 'Lunar Blast', ...
    'Color', [0 0 0], ...
...
% Draw the splash screen (if an image path was provided), then the welcome screen.
...
%% ========================================================================
%  Splash screen — shown after Subject ID, before planet selection
%  Displays a user-chosen image and a START button.
%  Clicking START removes this overlay and calls showWelcomeOverlay().
% ========================================================================
    function showSplashScreen(imagePath)
        % Create a full-figure axes for the splash.
        sax = axes('Parent', fig, ...
            'Position', [0 0 1 1], ...
        ...
        % Load and display the image, centred and scaled to fit.
        try
...
%% ========================================================================
%  Welcome overlay (runs inside the shared figure)
% ========================================================================
    function showWelcomeOverlay()
        % Welcome serial state (separate from game serial).
        wSerialObj   = [];
        wSerialTimer = [];
        wDialRaw     = 500;
...
% ========================================================================
    function buildGameScene()
        % Make the game axes visible and populate all graphic handles.
        set(ax, 'Visible', 'off');   % axes frame stays hidden; contents are visible
